...
var mouse_left_button_hold : bool = false
var arr_grid_placed_object = []
...
func selection_change():
	...
	if selected_objects.size() == 1:
		...
		if selected is BPB_Root_Basic:
			for obj in selected.get_children():
				var key = Vector3i(obj.global_position)
				arr_grid_placed_object.append(key)
...
func do_placement(viewport_camera, event):
	...
	if event is InputEventMouseMotion:
		if placement_options.grid: 
			###FIX GRID LIKE PLACEMENT BUT ATTACHED TO TOP OF COLLIDER HERE###---START###
			...
			if ray_result:
				var ray_pos : Vector3 = ray_result.position
				ray_pos = ray_pos.snapped(placement_options.grid_size)
				var ray_downward_result = _intersect_2m_downward(ray_pos + Vector3.UP)
				if ray_downward_result:
					var offset = ray_downward_result.position
					ghost.global_transform.origin = offset
					
					if mouse_left_button_hold:
						var key = Vector3i(ray_pos)
						if arr_grid_placed_object.find(key) == -1:
							place_object(ghost, placement_options)
							arr_grid_placed_object.append(key)
							stop_input_passthrough = true
			###FIX GRID LIKE PLACEMENT BUT ATTACHED TO TOP OF COLLIDER HERE###---END###
		else:
			match axis:
				_AXIS.SNAP:
					event_last_position = get_viewport().get_mouse_position()
	...
	if event is InputEventMouseButton :
		if event.is_pressed():
			if event.button_index == MOUSE_BUTTON_LEFT:
				mouse_left_button_hold = true
				if not placement_options.grid:
					place_object(ghost, placement_options)
				else:
					event_last_position = get_viewport().get_mouse_position()
					var ray_result = _intersect_with_colliders(viewport_camera, event.position)
...
func disable_ghost_collider(node):
	...
	for obj in node.find_children("*", "CollisionShape3D"):
		obj.shape = null
...
func _intersect_2m_downward(from):
	var dir = Vector3.DOWN * 2
